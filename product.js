...
let inbutton = document.querySelector("#in")
...
searchbutton.onkeyup = (e) => {
    ...
    let fil = e.target.value.toUpperCase()
    for (el of products) {
        if (el.title.toUpperCase().includes(fil)) {
            afficherProducts(products.filter(p => p.title.toUpperCase().includes(fil)))
        }
        else if (el.description.toUpperCase().includes(fil)) {
            afficherProducts(products.filter(p => p.description.toUpperCase().includes(fil)))
        }
        else if (el.category.toUpperCase().includes(fil)) {
            afficherProducts(products.filter(p => p.category.toUpperCase().includes(fil)))
...
inbutton.onclick = () => {
    afficherProducts(products.filter(p => p.stock > 0))
}
outbutton.onclick = () => {
    afficherProducts(products.filter(p => p.stock === 0))
}
